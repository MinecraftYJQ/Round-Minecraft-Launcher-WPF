...
namespace Round_Minecraft_Launcher.Cs.Launcher.Authenticator
{
    public class MicrosoftLogger
    {
        ...
        private static async Task<string[]> GetMicrosoftOAuthToken(string code)
        {
            ...
            {
                { "client_id", "b91b99f8-e1e9-4da6-966c-bb50a7bb6c47" },
                { "code", code },
            ...
            resultstrs[0] = jobj.RootElement.GetProperty("access_token").ToString() ?? throw new Exception("获取MS OAuth Token失败");
            resultstrs[1] = jobj.RootElement.GetProperty("refresh_token").ToString() ?? throw new Exception("获取Refresh Token失败");
            return resultstrs;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async static Task<string> FirstLogin(string code)
        {
